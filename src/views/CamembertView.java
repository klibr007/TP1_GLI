...
import controllers.CamembertController;
import controllers.IController;
import models.CamembertModel;
import models.ICamembertModel;
import models.PieModel;
import models.CamembertModelAdapter;
...
public class CamembertView extends JComponent implements MouseListener,
	...
	public CamembertView(ICamembertModel m) {
		...
		((CamembertModelAdapter)model).addObserver(this);
	...
	public void paintComponent(Graphics g) {
		...
		for (int i = 0; i < model.size(); i++) {
			...
			if (controller.isSelected() && controller.getSelectedPie() == i) {
				...
				arc.setAngleExtent((float)model.getValues(i) / (float)model.total() * 360
						- pieRadialGap);
				g2d.fill(arc);
			...
			angle += (float)model.getValues(i) / (float)model.total() * 360;
		}
		...
		if (!controller.isSelected()) {
			...
			for (int i = 0; i < model.size(); i++) {
				...
				double midangle = angle + (float)model.getValues(i) / (float)model.total()
						* 360.0 / 2.0;
		...
		if (controller.isSelected()) {
			...
			for (int i = 0; i < model.size(); i++) {
				...
				if (i == controller.getSelectedPie()) {
					...
					double midangle = angle + (float)model.getValues(i)
							/ (float)model.total() * 360.0 / 2.0;
...
public static void main(String[] a) {
        ...
        // Model model;
		ICamembertModel camembert = new CamembertModel(new String("Budget"), new String("Euros"), new String("Mon budget pour le mois"));
		camembert.addItem(new PieModel("cinema", "tr", 200));
		camembert.addItem(new PieModel("sortie", "cpr", 200));
		CamembertView view = new CamembertView(camembert);
		IController controller = new CamembertController(camembert, view);
